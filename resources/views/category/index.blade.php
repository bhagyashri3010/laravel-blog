				...
				<div class="row">
					<div class="col-md-12">
						<div class="card">
							...
							<div class="card-body">
								<table id="category_table" class="table table-bordered">
									...
									<!-- <tbody>
										...
										<tr>
											<td>
												<input type="checkbox" name="multi_delete[]" class="multi_del" data-id = "{{ $category['id']}}">
											</td>
											<td>{{ $category['name'] }}</td>
											<td>
												<a href="{{ url('categories/'. $category['id'].'/edit') }}" class="btn btn-info btn-xs">Edit</a>
												<a href="javascript:void(0)" data-toggle="modal" class="btn btn-xs btn-danger delete_single" data-id="{{ $category['id'] }}">Delete</a>
											</td>
										</tr>
										@endforeach
									</tbody> -->
							...
							<form id="del_form" action="{{url('categories/1')}}" method="POST">
				 				{{ csrf_field() }}
								<input type="hidden" name="_method" value="DELETE">
				 				<input id="hidden_del" type="hidden" name="hidden_del" value="">
				 			</form>
				 			<form id="multi_del_form" action="/multi-delete-category" method="POST">
				 				{{ csrf_field() }}
...
$(document).ready(function() {
	...
	$(document).on("click", "#multiple_delete_button", function(e) {
		if ($('input[name="multi_delete[]"]:checkbox:checked').length == 0) {
			...
			$(".multi_id").val(JSON.stringify(categories));
		}
	});
